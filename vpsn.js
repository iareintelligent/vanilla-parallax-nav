...
/***/ function(module, exports) {
	...
	this.sections={};this.links.map(function(link){if(link.hash){_this.sections[link.hash]=document.querySelector(link.hash).offsetTop-el.scrollHeight;link.addEventListener('click',function(e){e.preventDefault();_this.clickHandler(e);});}else {_this.sections['#top']=el.scrollHeight;}});}_createClass(ParallaxNav,[{key:'clickHandler',value:function clickHandler(e){this.scrollAnimate(this.sections[e.target.hash]);}},{key:'scrollAnimate',value:function scrollAnimate(end){var cur=window.scrollY,timer=setInterval(function(){if(cur==end){ //Stop animation
	clearInterval(timer);}else {cur>end?cur-=1:cur+=1;window.scrollTo(0,cur);}},15);}}]);return ParallaxNav;}();exports.default=ParallaxNav;window.parallaxNav=new ParallaxNav(document.querySelector('.vpsn'));
